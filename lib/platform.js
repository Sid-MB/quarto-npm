...
function resolveTarget(version = quartoVersion()) {
  ...
  // extracted root (VENDOR_DIR), which is `bin/quarto` (or `bin/quarto.exe`) on
  // every platform. The artifacts are inconsistent about wrapping: macOS and
  // Windows extract to `bin/` at the root, but the Linux tarballs wrap everything
  // in a `quarto-<version>/` directory. We no longer normalize this with tar's
  // `--strip-components` (some `tar` builds silently ignore it, which stranded the
  // launcher at `vendor/quarto-<version>/bin/quarto`); instead download.js
  // extracts as-is and locates + hoists the launcher's root into `vendor/`. So
  // this table only needs the asset name and the launcher's path within its root.
  let asset;
  let binRel;
